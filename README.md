...
- 全局鼠标手势：RDP 轨迹简化 + 连续方向/转角/形状联合匹配；仍用九宫格数字配置（如 `26` = 下·右）。
- 识别器使用固定栈数组与经典几何算法，无训练、无模型文件、无新增第三方运行库。
...
4. 托盘右键菜单：**重载配置 / 打开配置文件 / 开机自启 / 检查更新 / 项目地址 / 退出**。版本号见托盘图标悬浮提示。

## 配置（`hua.ini`，UTF-8）

方向数字：`8`上 `2`下 `4`左 `6`右 / `7`左上 `9`右上 `1`左下 `3`右下。
配置加载时会折叠连续重复方向（如 `266` 标准化为 `26`）；非法方向会被忽略并写入日志，
同一节内标准化后重复的手势采用最后定义的动作。

识别以完整二维轨迹为准，而不是简单比较方向字符串。末段已经达到 `MinDistance`、但尚未达到
自适应转向确认距离时，会作为“可能继续绘制”的证据参与候选比较：如果它指向一个已配置的
更长手势，程序会暂时拒绝短前缀，避免把未画完的 `39` 误执行成 `3`；若没有配置对应延伸手势，
小尾钩仍按噪声处理。分数不足或最佳、次佳太接近时同样拒识，不猜测动作。
...
| `MatchScore` | 几何匹配最低分，默认 `82`；越高越严格，推荐 `78`～`88` |
| `AmbiguityMargin` | 最佳候选至少领先第二名多少分，默认 `6`；不足则拒识 |
...
**排查**：配置没生效时先看 `hua.log`——拼错的键名、无法识别的开关值（开关只接受 `true`/`false`，也认 `1`/`0`、`yes`/`no`、`on`/`off`）、含非法方向或标准化后过长的手势、标准化后重复的手势，以及超出容量的条目都会记录在那里。重复手势采用同一节中最后定义的动作。
